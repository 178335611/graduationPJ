...
def train_model(model, dataloaders, dataset_sizes, criterion, optimizer,
                 device, paths, timestamp, resume_from=None, scheduler=None): # 删除 scheduler 参数
    """
    完整训练流程（支持断点续训与Plateau调度）

    Args:
        resume_from: 检查点路径，None则从头训练
        scheduler: 学习率调度器（推荐传入 ReduceLROnPlateau）
    """
    ...
    if resume_from and os.path.isfile(resume_from):
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            print("⚠️  注意：优化器和调度器状态无法恢复，将使用初始设置")
            start_epoch = 0
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        if scheduler is not None:
            # ReduceLROnPlateau 需要依赖一个监控指标，通常使用验证集的 loss
            current_val_loss = history['val_loss'][-1]

            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                scheduler.step(current_val_loss)
            else:
                # 兼容普通的调度器（如 StepLR, CosineAnnealingLR）
                scheduler.step()

        # 打印当前学习率 (放在 step 之后以显示更新后的 LR)
        current_lr = optimizer.param_groups[0]['lr']
        print(f"下一轮学习率: {current_lr:.6f}")
